...
async function main() {
  ...
  // 7. Finance user checks pending approvals
  ...
  const { data: pend } = await fin.from('orders').select('id, order_number, status, total_amount').eq('status', 'Pending Finance Approval').limit(5);
  console.log('Finance sees pending approvals:', JSON.stringify(pend));
