...
const PortalNavbar = ({ config, toggleSidebar }) => {
  ...
  const { name, accentHex, homeRoute } = config;
  const isHome = location.pathname === homeRoute || location.pathname === `/${name.toLowerCase()}`;
  const displayName = user?.full_name || user?.email?.split('@')[0] || 'User';

  // Page title from nav
  const currentPage = config.nav.find(n => location.pathname === n.path)?.name || name;
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="hidden md:flex items-center relative">
          <input
            type="text"
            placeholder="Search..."
            className="w-44 bg-slate-100 dark:bg-slate-900 border-none rounded-lg py-1.5 pl-8 pr-3 text-sm outline-none focus:ring-2 transition-all placeholder:text-slate-400"
