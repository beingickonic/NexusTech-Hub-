...
const getStatusBadge = (status) => {
  switch (status) {
    ...
    default:
      return 'bg-slate-50 text-slate-700 dark:bg-slate-500/10 dark:text-nexus-textSecondary border-slate-200 dark:border-slate-500/20';
  }
};
    ...
    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl border border-slate-200 dark:border-nexus-border shadow-sm overflow-hidden flex flex-col">
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan="6" className="px-6 py-12 text-center">
                  ...
                  <div className="text-nexus-textSecondary text-sm max-w-lg mx-auto bg-red-50 dark:bg-red-500/10 p-3 rounded-lg border border-red-100 dark:border-red-500/20 break-words">{errorMsg}</div>
                  <div className="mt-4 text-xs text-nexus-textSecondary">If you see an RLS error, ensure the SQL policies were added correctly and your user role is "Admin".</div>
                </td>
              </tr>
            ) : orders.length === 0 ? (
              <tr>
                <td colSpan="6" className="px-6 py-12 text-center text-nexus-textSecondary">No orders found.</td>
              </tr>
            ) : orders.map((order) => (
              <tr key={order.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors">
                ...
                <td className="px-6 py-4">
                  ...
                  <div className="text-xs text-nexus-textSecondary dark:text-nexus-textSecondary mt-0.5">{order.items} items</div>
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2 relative group">
                    <button onClick={() => setSelectedOrder(order)} className="p-2 text-nexus-textSecondary hover:text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-500/10 rounded-lg transition-colors" title="View Details">
                      <Eye size={16} />
                    </button>
                    <div className="relative">
                      <button className="p-2 text-nexus-textSecondary hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors">
                        <MoreVertical size={16} />
                      </button>
                      <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-nexus-border opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10 flex flex-col py-2">
                        <button onClick={() => window.print()} className="px-4 py-2 text-left text-sm text-slate-700 dark:text-nexus-textSecondary hover:bg-slate-50 dark:hover:bg-slate-700/50">Print Invoice</button>
                        <button onClick={() => window.print()} className="px-4 py-2 text-left text-sm text-slate-700 dark:text-nexus-textSecondary hover:bg-slate-50 dark:hover:bg-slate-700/50">Download PDF</button>
                        <button className="px-4 py-2 text-left text-sm text-slate-700 dark:text-nexus-textSecondary hover:bg-slate-50 dark:hover:bg-slate-700/50">Track Shipment</button>
                      </div>
                    </div>
                  </div>
                </td>
      ...
      {selectedOrder && (
        <>
          ...
          />
          <div className="fixed inset-y-0 right-0 w-full max-w-xl bg-white dark:bg-nexus-surface shadow-2xl z-50 transform transition-transform duration-300 flex flex-col border-l border-slate-200 dark:border-nexus-border">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-nexus-border bg-slate-50/50 dark:bg-nexus-surface/50">
              <div>
                ...
                <p className="text-sm text-nexus-textSecondary dark:text-nexus-textSecondary mt-1">
                  {selectedOrder.order_number || `#ORD-${selectedOrder.id.toString().padStart(5, '0')}`} • {formatDate(selectedOrder.date)}
                </p>
              </div>
              <button onClick={() => setSelectedOrder(null)} className="p-2 text-nexus-textSecondary hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:text-nexus-textSecondary rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-5 border border-slate-200 dark:border-nexus-border/50">
                <h4 className="font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                  <Package size={18} className="text-orange-500" /> Customer & Shipping
                </h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-nexus-textSecondary dark:text-nexus-textSecondary mb-1">Customer Name</p>
                    <p className="text-sm font-medium text-slate-900 dark:text-white">{selectedOrder.customer}</p>
                  </div>
                  <div>
                    <p className="text-xs text-nexus-textSecondary dark:text-nexus-textSecondary mb-1">Phone Number</p>
                    <p className="text-sm font-medium text-slate-900 dark:text-white">{selectedOrder.phone || 'N/A'}</p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-xs text-nexus-textSecondary dark:text-nexus-textSecondary mb-1">Shipping Address</p>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {(selectedOrder.order_items || []).map((item, idx) => (
                    <div key={idx} className="flex items-center gap-4 p-3 rounded-xl border border-slate-200 dark:border-nexus-border hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-nexus-textSecondary dark:text-nexus-textSecondary">Qty: {item.quantity}</p>
                      </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-5 border border-slate-200 dark:border-nexus-border/50">
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium text-nexus-textSecondary dark:text-nexus-textSecondary">Payment Status</label>
                    <select 
                      value={selectedOrder.payment_status || 'pending'}
                      onChange={(e) => handlePaymentStatusChange(selectedOrder.id, e.target.value)}
                      className="w-full bg-white dark:bg-nexus-surface border border-slate-200 dark:border-nexus-border rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white outline-none focus:ring-2 focus:ring-orange-500/50 capitalize"
                    >
                      <option value="pending">Pending</option>
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-medium text-nexus-textSecondary dark:text-nexus-textSecondary">Order Status</label>
                    <select 
                ...
                <div className="mt-5 pt-4 border-t border-slate-200 dark:border-nexus-border flex justify-between items-center">
              ...
              <div>
                ...
                <div className="space-y-4 pl-3 border-l-2 border-slate-200 dark:border-nexus-border ml-3">
                  <div className="relative">
                    ...
                    <p className="text-xs text-nexus-textSecondary">{formatDate(selectedOrder.date)}</p>
                  </div>
                  {(selectedOrder.payment_status === 'paid' || selectedOrder.payment_status === 'completed') && (
                    <div className="relative">
                      ...
                      <p className="text-xs text-nexus-textSecondary">Tracked in Payments</p>
                    </div>
                  )}
            ...
            <div className="p-4 border-t border-slate-200 dark:border-nexus-border bg-slate-50/50 dark:bg-nexus-surface/50 flex gap-3">
              <button onClick={() => window.print()} className="flex-1 py-2.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-lg text-sm font-medium border border-slate-200 dark:border-nexus-border hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                Print Invoice
              </button>
