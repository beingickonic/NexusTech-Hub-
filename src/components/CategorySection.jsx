...
const CategorySection = () => {
  ...
  return (
    <section className="py-14 sm:py-20 bg-white dark:bg-nexus-bg transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-end mb-8 sm:mb-10">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-nexus-heading mb-1 sm:mb-2">
              Shop by Category
            </h2>
            <p className="text-sm sm:text-base text-nexus-textSecondary dark:text-nexus-muted">
              Discover the tech that fits your lifestyle.
            </p>
          </div>
          <Link
            to="/products"
            className="hidden sm:flex items-center gap-2 text-primary font-medium hover:text-nexus-primary transition-colors text-sm"
          >
          ...
          {categories.map((cat, idx) => (
            ...
            >
              <Link to={`/products?category=${encodeURIComponent(cat.name)}`}>
                <div className="relative w-full aspect-[4/5] rounded-xl sm:rounded-2xl overflow-hidden bg-nexus-card shadow-md dark:shadow-soft mb-2 sm:mb-4 border border-nexus-border dark:border-transparent transition-all duration-300 hover:border-primary hover:shadow-primary/20 dark:hover:border-primary/50 dark:hover:shadow-glow">
                  <div className="absolute inset-0 bg-black/10 dark:bg-nexus-bg group-hover:bg-black/0 transition-colors duration-300 z-10" />
                  <img
                ...
                <h3 className="text-center font-semibold text-xs sm:text-sm text-nexus-heading group-hover:text-primary transition-colors line-clamp-1">
                  {cat.name}
                </h3>
              </Link>
            </motion.div>
