...
const STATUS_CONFIG = {
  ...
  returned:   { label: 'Returned',   color: 'text-slate-600 dark:text-slate-400',   bg: 'bg-slate-100 dark:bg-slate-500/15',   icon: ArrowRightLeft }
};
  ...
  <motion.button
    ...
    className={`text-left p-4 rounded-2xl border transition-all ${
      active
        ...
        : 'bg-white dark:bg-slate-800/60 border-slate-200 dark:border-slate-700/50 hover:border-slate-300 dark:hover:border-slate-600'
    }`}
  >
    ...
    <p className={`text-xs font-medium mt-0.5 ${active ? color : 'text-slate-500 dark:text-slate-400'}`}>{label}</p>
  </motion.button>
);
      ...
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-700">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Dispatch #{dispatch.dispatch_number}</p>
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {loading ? (
              <div className="text-center py-8 text-slate-400 text-sm">Loading drivers...</div>
            ) : filtered.length === 0 ? (
              <div className="text-center py-8 text-slate-400 text-sm">No available drivers</div>
            ) : filtered.map(driver => (
              <button key={driver.id} onClick={() => setSelectedDriver(driver)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 truncate">{driver.vehicle_type} · {driver.vehicle_number}</p>
                </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              return (
                <button key={s} onClick={() => setStatus(s)}
                  className={`flex items-center gap-2 p-2.5 rounded-xl border text-xs font-semibold transition-all ${
                    status === s ? `${cfg.bg} ${cfg.color} border-current` : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-slate-300'
                  }`}>
        ...
        <div className="p-5 border-t border-slate-200 dark:border-slate-700 flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800">Cancel</button>
...
const DispatchPage = () => {
  ...
  const statCards = [
    { key: 'all',       label: 'Total',      value: Object.values(stats).reduce((a,b) => a + (typeof b === 'number' ? b : 0), 0), icon: Truck,        color: 'text-slate-600 dark:text-slate-300', bg: 'bg-slate-100 dark:bg-slate-800' },
    ...
    <div className="animate-fade-in space-y-6 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Track and manage all delivery dispatches</p>
        </div>
            ...
            <tbody>
              {loading ? (
                ...
                <motion.tr key={d.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                  className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors group">
                  ...
                  <td className="px-4 py-3 max-w-[160px]">
                    <p className="text-xs text-slate-600 dark:text-slate-400 truncate">{d.delivery_address || 'N/A'}</p>
                  </td>
                  <td className="px-4 py-3">
                    {d.drivers ? (
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{d.drivers.vehicle_number}</p>
                        </div>
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    <p className="text-xs text-slate-600 dark:text-slate-400">{d.dispatch_date || '—'}</p>
                    {d.estimated_delivery && <p className="text-xs text-slate-400">Est: {d.estimated_delivery}</p>}
                  </td>
                  <td className="px-4 py-3">
