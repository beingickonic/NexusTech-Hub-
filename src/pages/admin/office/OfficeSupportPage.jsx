...
const StatCard = ({ title, value, icon: Icon, colorClass }) => (
  <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-5 rounded-2xl border border-slate-200 dark:border-nexus-border shadow-sm flex items-start justify-between">
    <div>
      <p className="text-xs font-semibold text-nexus-textSecondary dark:text-nexus-textSecondary mb-1 uppercase tracking-wider">{title}</p>
    ...
    <div className="animate-fade-in pb-10 flex flex-col h-[85vh]">
      <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-nexus-textSecondary dark:text-nexus-textSecondary">Manage incoming requests, visitors, calls, and general office queries.</p>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-2xl border border-slate-200 dark:border-nexus-border shadow-sm flex flex-col min-h-0">
        <div className="p-4 border-b border-slate-200 dark:border-nexus-border flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/50 dark:bg-nexus-surface/30">
          <div className="relative w-full sm:max-w-md">
            <input 
              ...
              className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-nexus-border rounded-lg py-2.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-orange-500/50 outline-none transition-all placeholder:text-nexus-textSecondary text-slate-900 dark:text-white"
            />
              ...
              {isLoading ? (
                ...
                filteredRequests.map(req => (
                  <tr key={req.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md text-xs font-medium text-slate-700 dark:text-nexus-textSecondary">
                        {req.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-slate-600 dark:text-nexus-textSecondary">
