...
const SupplierStatusBadge = ({ status }) => {
  ...
  }[status] || { label: status, color: 'text-slate-500', bg: 'bg-slate-100', dot: 'bg-slate-400' };
  return (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-end bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25 }}
        ...
        className="bg-white dark:bg-slate-900 w-full sm:w-[420px] h-full overflow-y-auto border-l border-slate-200 dark:border-slate-700 shadow-2xl">
        ...
        <div className="p-5 space-y-5">
            ...
            <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-3 space-y-2">
              {[
                ...
                <div key={i} className="flex items-start gap-2 text-sm text-slate-700 dark:text-slate-300">
                  <row.icon size={13} className="text-slate-400 flex-shrink-0 mt-0.5" />
                  <span className="break-all">{row.value}</span>
                </div>
              ))}
              {supplier.tax_number && (
                <div className="text-xs text-slate-500">Tax #: {supplier.tax_number}</div>
              )}
            </div>
          </div>
          ...
          {/* Products */}
          <div className="space-y-2">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-400">Products Supplied ({products.length})</h4>
            {loading ? (
              ...
              return (
                <div key={sp.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400">Stock: {product.stock || 0} · {sp.is_primary ? '★ Primary' : ''}</p>
                  </div>
                  {sp.unit_cost && <p className="text-xs font-bold text-slate-700 dark:text-slate-300">KES {sp.unit_cost}</p>}
                </div>
              );
            })}
          </div>
...
const SuppliersPage = () => {
  ...
  const statCards = [
    { label: 'Total Suppliers', value: stats.total || 0,    icon: Building2,  color: 'text-slate-700 dark:text-slate-300', bg: 'bg-slate-100 dark:bg-slate-800' },
    ...
    <div className="animate-fade-in space-y-6 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Manage your vendors and purchasing relationships</p>
        </div>
        <div className="flex gap-2">
          ...
          {[['all','All'], ['active','Active'], ['suspended','Suspended'], ['inactive','Inactive']].map(([k,l]) => (
            <button key={k} onClick={() => { setStatusFilter(k); setPage(1); }}
              className={`px-3 py-2 rounded-xl text-xs font-semibold transition-colors ${
                statusFilter === k ? 'bg-orange-500 text-white' : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
              }`}>{l}</button>
          ))}
        </div>
      </div>
            ...
            <motion.div key={supplier.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {supplier.company && <p className="text-xs text-slate-500 truncate">{supplier.company}</p>}
              ...
              <div className="space-y-1.5 mb-4">
                {supplier.contact_person && (
                  <div className="flex items-center gap-1.5 text-xs text-slate-500"><Users size={11} />{supplier.contact_person}</div>
                )}
                {supplier.phone && <div className="flex items-center gap-1.5 text-xs text-slate-500"><Phone size={11} />{supplier.phone}</div>}
                {supplier.email && <div className="flex items-center gap-1.5 text-xs text-slate-500 truncate"><Mail size={11} />{supplier.email}</div>}
              ...
              {/* Product count */}
              <div className="flex items-center gap-2 py-3 border-y border-slate-100 dark:border-slate-800 mb-4">
                <Layers size={13} className="text-slate-400" />
                <span className="text-xs text-slate-500">{supplier.products_count || 0} products linked</span>
              </div>
      ...
      {meta.totalPages > 1 && (
        <div className="flex justify-center gap-2">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={meta.page <= 1}
            className="px-4 py-2 rounded-xl text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 disabled:opacity-40">Prev</button>
          <span className="px-4 py-2 text-sm text-slate-500">Page {meta.page} / {meta.totalPages}</span>
          <button onClick={() => setPage(p => Math.min(meta.totalPages, p + 1))} disabled={meta.page >= meta.totalPages}
            className="px-4 py-2 rounded-xl text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 disabled:opacity-40">Next</button>
        </div>
      )}
