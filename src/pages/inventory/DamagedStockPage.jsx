...
const STATUS_CONFIG = {
  ...
  Approved:          { label: 'Approved',       color: 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400',        icon: CheckCircle2 },
  disposed:          { label: 'Disposed',       color: 'bg-slate-100 text-slate-600 dark:bg-white/5 dark:text-nexus-textSecondary',         icon: Trash2 },
  Disposed:          { label: 'Disposed',       color: 'bg-slate-100 text-slate-600 dark:bg-white/5 dark:text-nexus-textSecondary',         icon: Trash2 },
...
const ReportDamageModal = ({ onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-dark-surface rounded-2xl border border-slate-200 dark:border-nexus-border shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-nexus-border sticky top-0 bg-white dark:bg-dark-surface z-10">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-slate-50 dark:bg-white/[0.03] rounded-xl p-4 space-y-3">
            ...
            {record.action_type && <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-slate-700 dark:text-nexus-textSecondary">{record.action_type}</span>
            </div>}
          </div>
          {record.reason && <div className="bg-amber-50 dark:bg-amber-500/10 rounded-xl p-3"><p className="text-xs font-semibold text-amber-700 dark:text-amber-400 mb-1">Reason</p><p className="text-sm text-slate-700 dark:text-nexus-textSecondary">{record.reason}</p></div>}
          {record.notes  && <div className="bg-slate-50 dark:bg-white/5 rounded-xl p-3"><p className="text-xs font-semibold text-nexus-textSecondary mb-1">Notes</p><p className="text-sm text-slate-600 dark:text-nexus-textSecondary">{record.notes}</p></div>}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchRecords} className="p-2.5 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-nexus-border">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        {[
          ...
          { label: 'Disposed',      value: summaryStats.disposed,  color: 'text-nexus-textSecondary',  bg: 'bg-slate-50 dark:bg-white/5' },
      ...
      <div className="bg-white dark:bg-dark-surface rounded-2xl border border-slate-200 dark:border-nexus-border shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 dark:border-nexus-border flex flex-col sm:flex-row gap-4 items-center justify-between bg-slate-50/50 dark:bg-white/[0.02]">
          <div className="relative w-full sm:max-w-md">
            ...
            <input type="text" placeholder="Search product or reason..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-nexus-border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 text-slate-900 dark:text-white" />
          </div>
              ...
              {loading ? [...Array(4)].map((_, i) => (
                ...
                return (
                  <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-5 py-4"><span className="font-bold text-rose-600">{r.quantity}</span><span className="text-nexus-textSecondary text-xs ml-1">units</span></td>
                    <td className="px-5 py-4 max-w-[150px]"><p className="text-sm text-slate-600 dark:text-nexus-textSecondary truncate">{r.reason || '—'}</p></td>
                ...
                <tr><td colSpan={8} className="px-5 py-16 text-center text-nexus-textSecondary">
                  <AlertTriangle size={48} className="mx-auto text-nexus-textSecondary dark:text-slate-600 mb-3" />
