...
const STATUS_CONFIG = {
  ...
  cancelled:  { label: 'Cancelled',  color: 'bg-slate-100 text-slate-700 dark:bg-slate-500/20 dark:text-slate-400',   icon: XCircle }
};
...
const CreateTransferModal = ({ onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-dark-surface rounded-2xl border border-slate-200 dark:border-white/10 shadow-2xl w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1.5">From Warehouse *</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-500 uppercase mb-1.5">To Warehouse *</label>
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-dark-surface rounded-2xl border border-slate-200 dark:border-white/10 shadow-2xl w-full max-w-md">
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center justify-between">
            <span className="font-mono text-xs text-slate-500">{transfer.reference_number || `TRF-${transfer.id}`}</span>
          ...
          <div className="bg-slate-50 dark:bg-white/[0.03] rounded-xl p-4 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <p className="text-xs text-slate-500">{product.sku || ''}</p></div>
            </div>
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-1 text-slate-600 dark:text-slate-300"><MapPin size={14} /> {fromWh.name || 'N/A'}</div>
              <ArrowRight size={16} className="text-primary" />
              <div className="flex items-center gap-1 text-slate-600 dark:text-slate-300"><MapPin size={14} /> {toWh.name || 'N/A'}</div>
            </div>
            <div className="text-center"><span className="text-2xl font-bold text-primary">{transfer.quantity}</span><span className="text-slate-500 text-sm ml-1">units</span></div>
          </div>
          {transfer.notes && <p className="text-sm text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-white/5 rounded-lg p-3">{transfer.notes}</p>}
          ...
          {!showReject && (
            <div className="flex flex-wrap gap-2">
              ...
              {['pending','approved'].includes(transfer.status) && <button onClick={() => handleAction('cancelled')} disabled={loading} className="px-4 py-2 rounded-lg bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/10 text-sm font-medium disabled:opacity-60">Cancel</button>}
            </div>
          )}
        </div>
      </motion.div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage inventory movements between warehouses</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={fetchTransfers} className="p-2.5 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 transition-colors">
      ...
      <div className="bg-white dark:bg-dark-surface rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-slate-200 dark:border-white/10 flex flex-col sm:flex-row gap-4 items-center justify-between bg-slate-50/50 dark:bg-white/[0.02]">
          <div className="relative w-full sm:max-w-md">
            ...
            <input type="text" placeholder="Search product, warehouse, or ref..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/10 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 text-slate-900 dark:text-white" />
          </div>
              ...
              {loading ? [...Array(4)].map((_, i) => (
                ...
                <motion.tr key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-5 py-4">
                    ...
                    <p className="text-xs font-mono text-slate-500">{t.products?.sku || ''}</p>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4"><span className="font-bold text-slate-900 dark:text-white">{t.quantity}</span><span className="text-slate-400 text-xs ml-1">units</span></td>
                  <td className="px-5 py-4"><StatusBadge status={t.status} /></td>
                  <td className="px-5 py-4 text-xs text-slate-500">{new Date(t.created_at).toLocaleDateString()}</td>
                  <td className="px-5 py-4">
                    <button onClick={() => setSelectedTransfer(t)} className="p-2 hover:bg-primary/10 rounded-lg text-slate-400 hover:text-primary transition-colors">
                      <Eye size={16} />
                    </button>
                  </td>
                </motion.tr>
                ...
                <tr><td colSpan={7} className="px-5 py-16 text-center text-slate-500">
                  <ArrowRightLeft size={48} className="mx-auto text-slate-300 dark:text-slate-600 mb-3" />
