...
const AlertCard = ({ alert, onAcknowledge, onResolve }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -10 }}
      className={`bg-white dark:bg-dark-surface rounded-xl border border-slate-200 dark:border-white/10 border-l-4 ${sev.border} shadow-sm overflow-hidden ${alert.is_read ? 'opacity-70' : ''}`}>
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              ...
              {alert.suggested_action && (
                <p className="text-xs text-slate-400 mt-1.5 flex items-center gap-1"><CheckCircle2 size={11} /> {alert.suggested_action}</p>
              )}
            </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Monitor inventory health issues in real-time</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={fetchAlerts} className="p-2.5 rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10">
            <RefreshCw size={16} className="text-slate-600 dark:text-slate-300" />
          </button>
          {stats.unread > 0 && (
            <button onClick={handleMarkAllRead} className="inline-flex items-center gap-2 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors">
      ...
      <div className="flex flex-wrap items-center gap-3">
        {['active', 'acknowledged', 'resolved', 'all'].map(s => (
          <button key={s} onClick={() => setFilterStatus(s)}
            className={`px-4 py-2 rounded-xl text-sm font-semibold capitalize transition-colors border ${filterStatus === s ? 'bg-primary text-white border-primary shadow-lg shadow-primary/25' : 'bg-white dark:bg-dark-surface border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/5'}`}>
            {s}
          </button>
        ))}
        <select value={filterSeverity} onChange={e => setFilterSeverity(e.target.value)}
          className="ml-auto bg-white dark:bg-dark-surface border border-slate-200 dark:border-white/10 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 text-slate-900 dark:text-white">
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="h-24 bg-white dark:bg-dark-surface rounded-xl border border-slate-200 dark:border-white/10 animate-pulse" />
          ))}
        </div>
      ) : filtered.length > 0 ? (
        <AnimatePresence>
        ...
        <div className="text-center py-24">
          <Bell size={64} className="mx-auto text-slate-300 dark:text-slate-600 mb-4" />
