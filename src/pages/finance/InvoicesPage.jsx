      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-nexus-border overflow-hidden">
        <div className="p-4 border-b border-slate-200 dark:border-nexus-border flex gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search invoices..." className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 dark:border-nexus-border bg-slate-50 dark:bg-nexus-surface text-slate-900 dark:text-white focus:ring-2 focus:ring-rose-500" />
          </div>
        </div>
              ...
              {loading ? (
                ...
                invoices.map(invoice => (
                  <tr key={invoice.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="p-4 text-slate-600 dark:text-nexus-textSecondary">{invoice.profiles?.full_name || 'N/A'}</td>
                    <td className="p-4 text-slate-600 dark:text-nexus-textSecondary">KSh {Number(invoice.amount).toLocaleString()}</td>
                    <td className="p-4 text-slate-600 dark:text-nexus-textSecondary">KSh {Number(invoice.balance).toLocaleString()}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-slate-600 dark:text-nexus-textSecondary">{new Date(invoice.due_date).toLocaleDateString()}</td>
