      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-nexus-border overflow-hidden">
        <div className="p-4 border-b border-slate-200 dark:border-nexus-border flex gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search payments..." className="w-full pl-10 pr-4 py-2 rounded-xl border border-slate-200 dark:border-nexus-border bg-slate-50 dark:bg-nexus-surface text-slate-900 dark:text-white focus:ring-2 focus:ring-rose-500" />
          </div>
        </div>
              ...
              {loading ? (
                ...
                payments.map(payment => (
                  <tr key={payment.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="p-4 text-slate-600 dark:text-nexus-textSecondary">{new Date(payment.payment_date).toLocaleDateString()}</td>
                    <td className="p-4 font-medium text-slate-900 dark:text-white">{payment.finance_invoices?.invoice_number || 'N/A'}</td>
                    <td className="p-4 text-slate-600 dark:text-nexus-textSecondary">{payment.profiles?.full_name || 'N/A'}</td>
                    <td className="p-4 font-bold text-slate-900 dark:text-white">KSh {Number(payment.amount).toLocaleString()}</td>
                    <td className="p-4">
                      <span className="px-2 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-nexus-textSecondary">
                        {payment.method}
                      </span>
                    </td>
                    <td className="p-4 text-slate-600 dark:text-nexus-textSecondary">{payment.reference || '-'}</td>
                    <td className="p-4 flex gap-2">
                      <button className="p-1.5 text-nexus-textSecondary hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg"><Eye size={16} /></button>
                      {canEdit && (
                        <>
