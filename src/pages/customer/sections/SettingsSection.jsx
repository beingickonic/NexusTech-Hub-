...
const ToggleSwitch = ({ checked, onChange, label, description, icon: Icon }) => (
  <div className="flex items-center justify-between p-4 bg-white dark:bg-nexus-bg rounded-xl border border-slate-200 dark:border-[#1F2937]">
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-nexus-textSecondary dark:text-gray-500 text-xs mt-0.5">{description}</p>
      </div>
    </div>
    <button
      onClick={() => onChange(!checked)}
      className={`relative w-12 h-6 rounded-full transition-colors duration-200 flex-shrink-0 ${checked ? 'bg-nexus-primary' : 'bg-[#1F2937]'}`}
    >
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="mb-8">
        ...
        <p className="text-nexus-textSecondary dark:text-gray-400 text-sm mt-1">Manage your account security and preferences</p>
      </div>
      ...
      <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-[#1F2937] rounded-2xl p-6 mb-6">
        ...
        <form onSubmit={handlePasswordChange} className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-nexus-textSecondary dark:text-gray-500 uppercase tracking-wider font-semibold block mb-1.5">New Password</label>
              <input
            ...
            <div>
              <label className="text-xs text-nexus-textSecondary dark:text-gray-500 uppercase tracking-wider font-semibold block mb-1.5">Confirm Password</label>
              <input
      ...
      <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-[#1F2937] rounded-2xl p-6">
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={item.label} className="flex items-center justify-between p-4 bg-white dark:bg-nexus-bg rounded-xl border border-slate-200 dark:border-[#1F2937]">
              <div>
              ...
              <button
                ...
                className="text-xs text-nexus-textSecondary dark:text-gray-400 hover:text-slate-900 dark:text-white border border-slate-200 dark:border-[#1F2937] px-3 py-1.5 rounded-lg transition-colors"
              >
                {item.action}
              </button>
            </div>
