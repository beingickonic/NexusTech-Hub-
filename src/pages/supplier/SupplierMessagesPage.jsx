...
const SupplierMessagesPage = () => {
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-10 h-[calc(100vh-120px)] flex flex-col">
      ...
      <div className="flex-1 bg-white dark:bg-nexus-bg border border-nexus-border rounded-2xl overflow-hidden shadow-sm flex flex-col md:flex-row min-h-0">
        
        {/* Sidebar Channels */}
        <div className="w-full md:w-64 border-b md:border-b-0 md:border-r border-nexus-border bg-nexus-surface dark:bg-nexus-hover flex flex-col shrink-0">
          <div className="p-4 border-b border-nexus-border">
            <div className="relative">
              ...
              <input 
                type="text" 
                placeholder="Search..." 
                className="w-full pl-9 pr-4 py-2 bg-white dark:bg-nexus-bg border border-nexus-border rounded-xl text-sm focus:outline-none focus:border-nexus-primary"
              />
            </div>
          </div>
              ...
              <button 
                ...
                className={`w-full flex items-center justify-between p-3 rounded-xl text-sm font-medium transition-colors ${
                  activeChannel === channel.name 
                    ? 'bg-nexus-primary/10 text-nexus-primary dark:bg-nexus-primary/10 dark:text-nexus-primary' 
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="p-4 border-b border-nexus-border flex items-center gap-3 bg-white dark:bg-nexus-bg shrink-0">
            <div className="w-10 h-10 rounded-full bg-nexus-primary/10 dark:bg-nexus-primary/10 flex items-center justify-center text-nexus-primary">
              <MessageSquare size={20} />
            </div>
            <div>
              <h3 className="font-bold text-nexus-heading">{activeChannel}</h3>
              <p className="text-xs text-nexus-textSecondary">Usually replies within 1 hour</p>
            </div>
          </div>

          <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-nexus-surface/50 dark:bg-nexus-bg/50">
            {messages.map(msg => (
              <div key={msg.id} className={`flex flex-col ${msg.isMine ? 'items-end' : 'items-start'}`}>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-xs font-medium text-nexus-muted">{msg.sender}</span>
                  <span className="text-xs text-nexus-textSecondary">{msg.time}</span>
                </div>
                <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${
                  msg.isMine 
                    ? 'bg-nexus-primary text-white rounded-tr-sm' 
                    : 'bg-white dark:bg-nexus-dark-navy text-nexus-text border border-nexus-border rounded-tl-sm'
                }`}>
                  {msg.content}
                </div>
              </div>
          ...
          <div className="p-4 border-t border-nexus-border bg-white dark:bg-nexus-bg shrink-0">
            <div className="flex items-center gap-3">
              <input 
                type="text" 
                placeholder={`Message ${activeChannel}...`}
                className="flex-1 bg-nexus-surface dark:bg-nexus-hover border border-nexus-border rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-nexus-primary dark:text-white"
              />
              <button className="w-12 h-12 rounded-xl bg-nexus-primary hover:bg-nexus-primary-hover text-white flex items-center justify-center transition-colors shrink-0">
