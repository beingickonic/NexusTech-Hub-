  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">Manage your assigned deliveries</p>
      </div>

      <div className="space-y-4">
        {deliveries.length === 0 ? (
          <div className="bg-white dark:bg-dark-surface p-12 rounded-2xl border border-slate-200 dark:border-white/10 text-center">
            <Package size={48} className="mx-auto text-slate-300 mb-4" />
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">No deliveries yet</h3>
            <p className="text-slate-500">You don't have any deliveries assigned to you.</p>
          </div>
        ) : (
          deliveries.map(d => (
            <div key={d.id} className="bg-white dark:bg-dark-surface p-5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm flex flex-col md:flex-row gap-4 justify-between md:items-center">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-sm font-medium text-slate-500">#{d.dispatch_number}</span>
                </div>
                <div className="flex items-start gap-2 mb-1">
                  <MapPin size={16} className="text-slate-400 mt-1" />
                  <p className="font-semibold text-slate-900 dark:text-white">{d.delivery_address}</p>
                </div>
                {d.notes && <p className="text-sm text-slate-500 ml-6">Note: {d.notes}</p>}
              </div>

              <div className="flex items-center gap-2">
                {d.status === 'assigned' && (
                  <button onClick={() => updateStatus(d.id, 'picked_up')} className="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-sm font-bold rounded-xl hover:opacity-90 transition-opacity">
                    Pick Up Package
                  </button>
                )}
                {d.status === 'picked_up' && (
