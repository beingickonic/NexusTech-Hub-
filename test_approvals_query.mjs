...
async function main() {
  ...
  console.log("Logged in. Executing getOrdersAwaitingInventoryApproval query...");
  ...
  if (error) {
    ...
    console.log("Query succeeded! Found", data.length, "orders.");
    if (data.length > 0) {
      console.log("First order example:", JSON.stringify(data[0]));
    }
  }
}
